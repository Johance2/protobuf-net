...
#pragma warning restore CS0618
        ...
        [Fact]
        public void CommentsAreEmittedInCSharpOutput()
        {
            var result = CSharpCodeGenerator.Default.Compile(new CodeFile("comments.proto", @"syntax=""proto3"";
// message comment
message Foo {
  // field comment
  int32 bar = 1; // inline field comment
}"));

            Assert.Empty(result.Errors);
            var text = result.Files.Single().Text;
            Assert.Contains("/// <summary>", text);
            Assert.Contains("message comment", text);
            Assert.Contains("field comment", text);
            Assert.Contains("inline field comment", text);
        }
